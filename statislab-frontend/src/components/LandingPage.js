...
function LandingPage({ steps = [], onEnterWorkspace }) {
  ...
  return (
    <main style={{ fontFamily: "Inter, sans-serif", background: "linear-gradient(160deg, #f6fbfa 0%, #f8fafc 55%, #eef6f3 100%)", color: "#1f2937", minHeight: "100vh" }}>
      ...
      >
        <div>
          <span style={{ display: "inline-block", fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#0f766e", background: "#dff5ee", padding: "4px 12px", borderRadius: 100, marginBottom: "1.25rem" }}>
            Statistical analysis
          </span>
          <h1 style={{ fontSize: "clamp(2rem, 3.2vw, 2.9rem)", fontWeight: 600, lineHeight: 1.2, letterSpacing: "-0.03em", color: "#1f2937", margin: "0 0 1.25rem" }}>
            Turn raw datasets into{" "}
            <span style={{ color: "#0f766e" }}>cleaned outputs,</span>{" "}
            tests, and reports.
          </h1>
          <p style={{ fontSize: isMobile ? 15 : 16, lineHeight: 1.75, color: "#475569", margin: "0 0 2rem", maxWidth: "46ch" }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
            <button
              ...
              style={{ background: "#0f766e", color: "#fff", border: "none", padding: "11px 22px", borderRadius: 8, fontFamily: "Inter, sans-serif", fontSize: 15, fontWeight: 500, cursor: "pointer" }}
            >
              Enter Workspace
            </button>
            <a href="#landing-flow" style={{ fontSize: 15, color: "#0f766e", textDecoration: "none", fontWeight: 500 }}>
              See the workflow →
            </a>
          </div>
        </div>
        ...
        <div style={{ background: "linear-gradient(145deg, #ffffff, #f4fbf8)", border: "1px solid rgba(15,118,110,0.16)", borderRadius: 16, padding: isMobile ? "1.25rem" : "2rem" }}>
          <p style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#0f766e", margin: "0 0 1.25rem" }}>
          ...
          <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: "1rem" }}>
            {[
              ...
              <li key={item} style={{ display: "flex", alignItems: "flex-start", gap: 10, fontSize: 15, color: "#1f2937", lineHeight: 1.55 }}>
      ...
      >
        {FEATURE_CARDS.map((card) => (
          <article key={card.title} style={{ background: "#fff", border: "1px solid rgba(15,118,110,0.12)", borderRadius: 12, padding: "1.75rem" }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: "#0f766e", letterSpacing: "0.08em", margin: "0 0 1rem" }}>{card.num}</p>
            <h2 style={{ fontSize: 16, fontWeight: 600, color: "#1f2937", margin: "0 0 0.6rem", letterSpacing: "-0.01em" }}>{card.title}</h2>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "#475569", margin: 0 }}>{card.description}</p>
          </article>
        ))}
      </section>
      ...
      <section id="landing-flow" style={{ padding: isMobile ? "2rem 1rem" : "4rem 3rem", background: "#fff", borderTop: "1px solid rgba(15,118,110,0.12)" }}>
        <div style={{ marginBottom: "2.5rem" }}>
          <span style={{ display: "inline-block", fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#0f766e", background: "#dff5ee", padding: "4px 12px", borderRadius: 100, marginBottom: "0.75rem" }}>
            Workflow preview
          </span>
          <h2 style={{ fontSize: "1.5rem", fontWeight: 600, color: "#1f2937", margin: 0, letterSpacing: "-0.02em" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(175px, 1fr))", gap: "1.5rem" }}>
          {steps.map((step) => (
            <div key={step.number} style={{ padding: "1.25rem", background: "linear-gradient(145deg, #f6fbfa, #fff)", border: "1px solid rgba(15,118,110,0.12)", borderRadius: 10 }}>
              <p style={{ fontSize: 12, fontWeight: 600, color: "#0f766e", letterSpacing: "0.08em", margin: "0 0 0.6rem" }}>0{step.number}</p>
              <h3 style={{ fontSize: 14, fontWeight: 600, margin: "0 0 0.35rem", color: "#1f2937" }}>{step.label}</h3>
              <p style={{ fontSize: 13, lineHeight: 1.6, color: "#475569", margin: 0 }}>{step.hint}</p>
            </div>
          ))}
        </div>
      </section>
